...
def main():
    ...
    harvested_temp_dir = r"E:\IoT_HeatIsland_Data\data\NYC\dataHarvest_NYC_202001_03" + "\\"
    ...
    interpolated_dir = processed_dir + "interpolated_NYC\\"
    try:
        # Create target Directory
    ...
    result_matrix = interpolated_dir + r'\tempMatrix_NYC.csv'
    ...
    # interpolated_dir = r'E:\IoT_HeatIsland_Data\data\LA\dataHarvest\dataHarvest_LA_201907_12\processed\interpolated_LA'
    # print('\n##### start building temp matrix #####')
    # # utils.fname_change(interpolated_dir)  # change files name to sort by date
    # files = glob.glob(interpolated_dir + '\\20*.csv')
    # # lsorted = sorted(files)  # sort by date
    # # print(lsorted)
    #
    # '''2D matrix (sensor * time)'''
    # df = pd.concat([utils.my_csv_reader(f) for f in files], axis=1, join='inner')
    # print(df)
    #
    # '''2D matrix (time * sensor)'''
    # result_matrix = r'E:\IoT_HeatIsland_Data\data\LA\dataHarvest\dataHarvest_LA_201907_12\processed\interpolated_LA\tempMatrix_LA.csv'
    # df1_transposed = df.T
