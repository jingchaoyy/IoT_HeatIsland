...
def rect_detect(left_down, length, width):
    ...
    print('length: %s width: %s' % (length, width))
    ...
    for i in range(width):
        for j in range(length):
            if [leftdown[0] + j, leftdown[1] + i] not in coor_list:
                ...
                rect_geohashs.append(geohash_list[coor_list.index([left_down[0] + j, left_down[1] + i])])
...
if __name__ == '__main__':
    ...
    np.save('../../IoT_HeatIsland_Data/data/LA/exp_data/processed/all_pred.npy', all_pred)
    all_truth = temperature[:, :, 1:1 + output_time_length * pred_times_num]
    all_truth = all_truth.transpose(2, 0, 1)
    np.save('../../IoT_HeatIsland_Data/data/LA/exp_data/processed/all_truth.npy', all_truth)
    print('npy saved!')
