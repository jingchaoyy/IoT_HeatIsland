...
def retry_if_connection_error(exception):
    ...
    print(exception)
    return True
# if exception retry with 10 second wait
        ...
        for f in fxx:
            ...
            if more_vars == 0:
                ...
                outfile = '%s/%sfromPando_%s_h%02d_f%02d_%s.grib2' \
                          % (outdir, model.upper(), DATE.strftime('%Y%m%d'), h, f,
                ...
                outfile = '%s/%sfromPando_%s_h%02d_f%02d_%s-and-%s.grib2' \
                          % (outdir, model.upper(), DATE.strftime('%Y%m%d'), h, f,
                             variable.replace(':', '_').replace(' ', '_'), more_vars)
            ...
            if os.path.isfile(outfile):
                ...
                lines = safe_request(idxfile).text.split('\n')
...
eDATE = date(2020, 4, 15)   # End date (exclusive)
# list of variables
# variables = ['TMP:2 m', 'UGRD:10 m', 'VGRD:10 m', 'RH:2 m', 'PRES:surface', 'HPBL', 'WRF']
variables = ['UGRD:10 m', 'PRES:surface']
...
for DATE in DATES:
    for variable in variables:
        download_HRRR_variable_from_pando(DATE, variable,
                                          hours=range(0, 24), fxx=[0],
                                          model='hrrr', field='sfc',
                                          outdir=r'D:\1_GitHub\IoT_HeatIsland_Data\data_hrrr\all_surface')