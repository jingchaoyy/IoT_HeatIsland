...
def main():
    ...
    for coor in coors:
        ...
        model.add(
            LSTM(configs['model']['layers'][2]['neurons'],
                 return_sequences=configs['model']['layers'][2]['return_seq']))
        model.add(
            LSTM(configs['model']['layers'][3]['neurons'],
                 return_sequences=configs['model']['layers'][3]['return_seq']))
        model.add(Dropout(configs['model']['layers'][4]['rate']))
        model.add(
            Dense(configs['model']['layers'][5]['neurons'], activation=configs['model']['layers'][5]['activation']))
        ...
        '''save to csv'''
        result = pd.DataFrame(index=np.arange(max(len(y_test), len(predictions))))
        result['tru'] = pd.Series(y_test.reshape(y_test.size))
        result['prediction'] = pd.Series([j for i in predictions for j in i])
        result.to_csv('../IoT_HeatIsland_Data/data/LA/exp_data/result_single_point_prediction_LSTM/' + coor + '.csv')
